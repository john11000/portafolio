	  ...
	  const cb = (entries) => {
		...
		entries.forEach((entry) => {
		  if (entry.isIntersecting) {
			...
			document.querySelector(elementosObservados[elemento]).classList.toggle("animate__animated");
			document
			  .querySelector( elementosObservados[elemento])
			  .classList.toggle("animate__bounceInDown");
			document.querySelector(elementosObservados[elemento]).classList.toggle("animate__delay-1s");
		  }
		});
	  };
